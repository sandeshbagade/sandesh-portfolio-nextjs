      ...
      <div className='container mx-auto px-4 md:px-6'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-8'>
          {skills.map((skillCategory, index) => (
            <motion.div
              key={index}
              className='bg-white dark:bg-gray-800 rounded-xl shadow-md p-5 md:p-6 border border-gray-200 dark:border-gray-700'
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.5, delay: index * 0.1 }}
            >
              <h3 className='text-lg sm:text-xl font-bold text-gray-900 dark:text-white mb-3 md:mb-4'>
